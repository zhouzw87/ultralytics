...
import sys
import os
sys.path.insert(0, os.path.abspath("../trains"))
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        input_image = torch.full((3, 640, 640), 114 / 255.0, dtype=torch.float32)
            ...
            if save_json:
                ...
                for p, b in zip(pred.tolist(), box.tolist()):
                    jdict.append({'image_id': image_id,
                                  'category_id': names[int(p[5])] if is_coco else int(p[5]),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', type=str, default='../trains/modified/modified.onnx.engine', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='../ultralytics/cfg/datasets/coco-test.yaml', help='*.data path')
